...
const arrowIconSVG = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><line x1="4" y1="12" x2="20" y2="12"/><polyline points="13 5 20 12 13 19"/></svg>`;
  ...
  galleryGrid.innerHTML = SITE_DATA.photos.map((p, i) => `
    <button type="button" class="gallery-item is-shown" data-album="${p.album}" data-index="${i}" style="transition-delay:${(i % 6) * 60}ms">
      ...
      <span class="gallery-overlay">
        <span class="gallery-overlay-discover">Discover</span>
        <span class="gallery-overlay-row">
          <span class="gallery-overlay-title">${p.title}</span>
          <span class="gallery-overlay-arrow">${arrowIconSVG}</span>
        </span>
      </span>
    </button>
  `).join('');
